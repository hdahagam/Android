...
//        fT.addToBackStack(null);
//        fT.commit();
        MovieDataFragment movieDataFragment = MovieDataFragment.newInstance(movie);
        movieDataFragment.setSharedElementEnterTransition(new DetailsTransition());
        movieDataFragment.setSharedElementReturnTransition(new DetailsTransition());
        ImageView img = (ImageView) v.findViewById(R.id.imgView);
        FragmentManager fm = getSupportFragmentManager();
        FragmentTransaction fT = fm.beginTransaction();
        fT.addSharedElement(img,img.getTransitionName());
        fT.replace(R.id.task1_framelayout, movieDataFragment);
        fT.addToBackStack(null);
        fT.commit();
    }
}
